...
const { loadWorkspaceProjectCatalog, moveOrphanScriptMetaForFile, readLocalProjectState, readLocalProjectStateAsync, readWorkspaceConfig, resolveProjectSelectionForPlace, writeStudioProjectState } = require("./project");
...
const { BRIDGE_TOKEN_HEADER, HttpError, SESSION_TOKEN_HEADER, errorResponse, jsonResponse, normalizeToken, readJsonBody, timingSafeEqualString } = require("./http-utils");
const { handleTool: handleMcpTool } = require("./mcp");
const { createMcpShieldState, listTools, mcpShieldSummary, mcpToolResultToHttpPayload } = require("./mcp-shield");
const { AMARILLO_PROTOCOL_VERSION, DAEMON_VERSION, normalizeProtocolVersion, normalizeVersion } = require("./version");
...
class PluginRobloxApp {
    ...
    handleProjectDefinitionChanged(changedProjectId) {
        ...
        for (const session of this.sessions.values()) {
            ...
            this.scheduleProjectTreeApply(session, project, "workspace_project_changed", path.join(this.workspaceRoot, changedProjectId), 50);
        }
    }
    ...
    acceptConnection({ offerId = null, studioInstanceId = null, placeId = 0, projectId = null, truthSource = "pc", pluginVersion = null, pluginProtocolVersion = null, requirePluginVersion = false }) {
        if (offerId) {
            if (!this.connectionOffer || this.connectionOffer.offerId !== offerId) {
                return {
                    ok: false,
    ...
    enqueueCommand(sessionId, type, payload, waitForResult = false, timeoutMs = waitForResult ? COMMAND_RESULT_TIMEOUT_MS : SYNC_COMMAND_TIMEOUT_MS) {
        const session = this.sessions.get(sessionId);
        if (!session) {
    ...
    calculateDiff(studioSnapshot, pcSnapshot, truthSource) {
        ...
        const sMounts = {};
        for (const m of (studioSnapshot.mounts || []))
            sMounts[m.id] = m;
        const pMounts = {};
        for (const m of (pcSnapshot.mounts || []))
            pMounts[m.id] = m;
        ...
        try {
            ...
            if (request.method === "GET" && requestUrl.pathname === "/errors") {
                ...
                if (since)
                    filters.since = since;
                if (sessionIdFilter)
                    filters.sessionId = sessionIdFilter;
