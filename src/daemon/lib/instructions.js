...
function buildInstructionsMarkdown(context = {}) {
    ...
    const projectLines = (context.projects || []).map((project) => (`- \`${project.id}\` (${project.name || "unnamed"})`));
    ...
    return [
        ...
        "## Native MCP Workflow",
        "",
        "- `Amarillo: Start Bridge` also writes or updates `.vscode/mcp.json` for the current workspace.",
