...
function loadWorkspaceProjectCatalog(workspaceRoot) {
    ...
    function markCycle(cycleDescriptors) {
        ...
        addIssue(createIssue("PROJECT_EXTENDS_CYCLE", cycleDescriptors[0], `Project '${cycleDescriptors[0].id}' has an inheritance cycle: ${cycleLabel}.`, { cycle: cycleLabel }));
    }
    function resolveDescriptor(descriptor, stack = []) {
        ...
        if (descriptor.extendsAbsolutePath) {
            ...
            if (!parentDescriptor) {
                ...
                addIssue(createIssue("PROJECT_EXTENDS_NOT_FOUND", descriptor, `Project '${descriptor.id}' references a missing base at '${normalizeSlashes(descriptor.extendsRaw)}'.`, { extendsProjectPath: normalizeSlashes(descriptor.extendsRaw) }));
                return null;
            }
            parentProject = resolveDescriptor(parentDescriptor, stack.concat(descriptor));
            if (!parentProject) {
                descriptor.resolutionState = "invalid";
                addIssue(createIssue("PROJECT_EXTENDS_INVALID", descriptor, `Project '${descriptor.id}' depends on an invalid base: '${parentDescriptor.id}'.`, { baseProjectId: parentDescriptor.id }));
                return null;
            }
        }
