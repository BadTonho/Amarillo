...
async function getSidebarState() {
    ...
    if (activeSession) {
        ...
        sessionFacts.push(createSidebarFact("Project", activeSession.projectName, isFallbackProjectSelection(activeSession) ? "warning" : "success"), createSidebarFact("State", activeSession.connectionState || "ready", sessionTone), createSidebarFact("Version", activeSession.versionState || "unknown", activeSession.requiresPluginUpdate ? "danger" : "success"), createSidebarFact("Sync", activeSession.syncState || "ready", activeSession.requiresManualResync || activeSession.requiresPluginUpdate ? "warning" : "success"), createSidebarFact("Place", String(activeSession.placeId || 0)), createSidebarFact("Session", activeSession.id), createSidebarFact("Ready sessions", String(readySessions.length), readySessions.length > 0 ? "success" : "neutral"));
        if (activeSession.studioContactState && activeSession.studioContactState !== "fresh") {
            sessionFacts.push(createSidebarFact("Studio", studioContactLabel(activeSession), studioContactTone(activeSession)));
        }
        ...
        if (isFallbackProjectSelection(activeSession)) {
            sessionFacts.push(createSidebarFact("Routing", describeProjectSelection(activeSession), "warning"));
        }
        ...
        sessionFacts.push(createSidebarFact("Status", "Daemon from another workspace", "warning"), createSidebarFact("Daemon", workspaceDisplayName(health.workspaceRoot), "warning"));
        if (visibleWorkspaceRoot) {
        ...
        sessionFacts.push(createSidebarFact("Status", handshakeStatusLabel(connectionOffer), sessionTone));
        if (visibleWorkspaceRoot) {
            sessionFacts.push(createSidebarFact("Workspace", workspaceDisplayName(visibleWorkspaceRoot)));
        }
    }
    ...
    if (settings || health?.ok) {
        if (mcpShield) {
            sessionFacts.push(createSidebarFact("MCP", mcpStateLabel(mcpShield), mcpStateTone(mcpShield)));
        }
        sessionFacts.push(createSidebarFact("VS Code -> Studio", autoSyncToStudio ? "Auto" : "Manual", autoSyncToStudio ? "success" : "warning"));
    }
    return {
        status: {
...
async function chooseSession(forcePick = false) {
    ...
    const picked = await vscode.window.showQuickPick(sessions.map((session) => sessionQuickPickItem(session, activeSessionId)), {
...
async function handleSidebarVisible(context) {
    ...
    if (!health?.ok) {
        ...
        const choice = await vscode.window.showInformationMessage("The Amarillo bridge is offline. Start it now and request a connection from Roblox Studio?", "Iniciar");
        if (choice === "Iniciar") {
            try {
...
async function runHealthcheck() {
    ...
    refreshSidebar();
    outputChannel.show(true);
    vscode.window.showInformationMessage(`Amarillo OK. Daemon workspace: ${daemonWorkspaceLabel}. Projects: ${payload.projectCount ?? 0}. Sessions: ${payload.sessions?.length ?? 0}.${autoSyncInfo}${mcpInfo}${projectWarning}${offerInfo}${workspaceWarning}`);
}
...
function activate(context) {
    ...
    context.subscriptions.push(outputChannel, statusBar, vscode.window.registerWebviewViewProvider("amarillo.sidebar", sidebarProvider), vscode.commands.registerCommand("amarillo.installRobloxPlugin", () => installRobloxPlugin(context)), vscode.commands.registerCommand("amarillo.startBridge", () => startBridge(context)), vscode.commands.registerCommand("amarillo.stopBridge", () => stopBridge()), vscode.commands.registerCommand("amarillo.healthcheck", () => runHealthcheck()), vscode.commands.registerCommand("amarillo.doctor", () => runDoctor()), vscode.commands.registerCommand("amarillo.mcpHealthcheck", () => runMcpHealthcheck()), vscode.commands.registerCommand("amarillo.configureMcp", () => configureMcp(context)), vscode.commands.registerCommand("amarillo.openOutput", () => outputChannel.show(true)), vscode.commands.registerCommand("amarillo.refreshSidebar", () => refreshSidebar()), vscode.commands.registerCommand("amarillo.sendFilesToStudio", async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("amarillo.openMenu", async () => {
        const menuItems = [
        ...
        try {
            switch (picked.action) {
                ...
                case "selectSession":
                    await selectSession();
                    break;
                case "sendFilesToStudio":
                    await sendFilesToStudio();
                    break;
                case "receiveFilesFromStudio":
                    await receiveFilesFromStudio();
                    break;
                case "execCode":
                    await executeCodeInStudio();
                    break;
                case "installRobloxPlugin":
                    await installRobloxPlugin(context);
                    break;
                case "configureMcp":
                ...
                case "openOutput":
                    ...
                    break;
                case "settings":
                    vscode.commands.executeCommand("workbench.action.openSettings", "amarillo");
                    break;
            }
        }
        catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("amarillo.execCode", async () => {
        try {
            await executeCodeInStudio();
        }
        catch (error) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider(tomlSelector, {
